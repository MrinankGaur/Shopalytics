...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // --- THIS IS THE FIX ---
        // Instead of a client-side router.push, we perform a full page
        // navigation. This forces the browser to make a fresh request to the
        // dashboard, ensuring the newly set authentication cookie is included.
        ...
        <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-lg border">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2 mt-1 border rounded-lg" required placeholder="you@example.com" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700">Password</label>
                    <PasswordInput value={password} onChange={(e) => setPassword(e.target.value)} placeholder="More than 6 characters" />
                </div>
